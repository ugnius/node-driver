	...
	for ( i = 0; i < rows_count; i++ ) {
		...
		for ( j = 0; j < columns_count; j++ ) {
			...
			if ( valueSize >= 0 ) {
				...
				value._o = 0;
				body._o += valueSize;
				value = Message._deserializeValue(value, col_specs[j].typeId, col_specs[j].firstTypeId, col_specs[j].secondTypeId);
			}
...
handle[eColumnType.SET] = function(v, f) {
	...
	for ( i = 0; i < count; i++) {
		...
		v._o += size;
		result.push( Message._deserializeValue( value, f, null, null ) );
	}
...
handle[eColumnType.MAP] = function(v, f, s) {
	...
	for (i = 0; i < count; i++) {
		...
		value = v.slice(v._o, v._o + size);
		v._o += size;
		key = Message._deserializeValue( value, f, null, null );

		size = util.readShort(v);
		value = v.slice(v._o, v._o + size);
		v._o += size;

		result[key] = Message._deserializeValue( value, s, null, null );
	}
...
Message._deserializeValue = function (value, columnTypeId, firstTypeId, secondTypeId) {

	if ( !handle[columnTypeId] ) {
		console.log('Unknown typeId ' + columnTypeId + ' ' + eColumnTypeName[columnTypeId] + ' to serialze');
	} else {
		value = handle[columnTypeId](value, firstTypeId, secondTypeId);
	}
